...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://docs.buildingsmartalliance.org/nbims03/cobie/schematron/functions" xmlns:cfn="http://docs.buildingsmartalliance.org/nbims03/cobie/schematron/functions" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	<xsl:key name="AttributeConstraintKey" match="AttributeConstraints/SheetAttributeConstraints" use="normalize-space(lower-case(@sheetName))"/>
	...
	<xsl:function name="cfn:attributeConstraintsCheck" as="xs:string">
		<xsl:param name="context"/>
		<!--The context of the check -->
		<xsl:param name="attributeRowName"/>
		<!--The context's corresponding column for the Attribute Sheet Name column-->
		<xsl:choose>
			<xsl:when test="lower-case(cfn:WorksheetName($context))='attribute'">
				<xsl:value-of select="'__NONE__'"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="sheetAttributeConstraints" select="key('AttributeConstraintKey',lower-case(cfn:WorksheetName($context)),$context/parent::*/parent::*)">
				</xsl:variable>
				<xsl:variable name="Results">
					<xsl:for-each select="$sheetAttributeConstraints[1]/AttributeConstraint">
							<xsl:choose>
								<xsl:when test="cfn:attributeConstraintCheck(.,$context,$attributeRowName)=false()">
				...
				<xsl:choose>
					<!--<xsl:when test="count($Results/Result[string-length(text()) &gt; 0 and not(text()='__NONE__')]) > 0">-->
					<xsl:when test="string-length($Results) &gt; 0">
						<xsl:value-of select="string-join($Results,', ')"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of>__NONE__</xsl:value-of>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
	<xsl:function name="cfn:attributeConstraintCheck" as="xs:boolean">
		<!--**This function returns true if the given cobie row passes the attribute constraint check and false otherwise -->
		<xsl:param name="attributeConstraint"/>
		<!--**An attribute.cobie.prairiesky.com AttributeConstraint element-->
		<xsl:param name="context"/>
		<!--**The context of the check - i.e. the COBie data element that we are running the check on -->
		<xsl:param name="attributeRowName" as="xs:string"/>
		<xsl:variable name="worksheetName" as="xs:string">
			<xsl:value-of select="cfn:WorksheetName($context)"/>
		</xsl:variable>
		<xsl:variable name="ruleApplies">
			<xsl:choose>
				<xsl:when test="$attributeConstraint/SheetConditions">
					<xsl:value-of select="cfn:attributeConstraintSheetConditionsCheck($attributeConstraint/SheetConditions,$context)"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="$ruleApplies=true()">
				<xsl:variable name="AttributeChecks">
					<xsl:for-each select="$attributeConstraint/AttributeName">
					<!--	<xsl:element name="AttributeCheck">-->
							<xsl:choose>
								<xsl:when test="./@requiresValue=true()">
									<xsl:value-of select="cfn:hasAttributeAndValue($context,./text(),$worksheetName,$attributeRowName)"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="cfn:hasAttribute($context,./text(),$worksheetName,$attributeRowName)"/>
								</xsl:otherwise>
							</xsl:choose> 
					<!--	</xsl:element> -->
					</xsl:for-each>
				...
				<xsl:choose>
				<!--	<xsl:when test="count($AttributeChecks[AttributeCheck=false()]) &gt; 0">-->
				<xsl:when test="contains(lower-case($AttributeChecks),'false')">
	...
	<xsl:function name="cfn:attributeConstraintSheetConditionsCheck" as="xs:boolean">
		<!--**This function returns true if the given cobie element passes the attribute sheet conditions-->
		<xsl:param name="sheetConditions"/>
		<!--**An attribute.cobie.prariesky.com AttributeConstraint/SheetConditions element-->
		<xsl:param name="context"/>
		<!--**The context of the check (i.e. cobie data element)-->
		<xsl:variable name="logicalOperator" as="xs:string">
			<xsl:choose>
				<xsl:when test="$sheetConditions/@LogicalOperator">
					<xsl:value-of select="$sheetConditions/@LogicalOperator"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="ConditionsEvaluation">
			<xsl:for-each select="$sheetConditions/Condition">
				<xsl:variable name="casedColumnValue">
					<xsl:choose>
						<xsl:when test="@caseSensitive=true()">
							<xsl:value-of select="ColumnValue"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="upper-case(ColumnValue)"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
				<xsl:variable name="ColumnName" select="ColumnName"/>
				<xsl:variable name="contextColumnValue">
					<xsl:choose>
						<xsl:when test="$context/child::*[upper-case(local-name()) =upper-case($ColumnName)][1]">
							<xsl:choose>
								<xsl:when test="@caseSensitive=true()">
									<xsl:value-of select="$context/child::*[upper-case(local-name()) =upper-case($ColumnName)][1]"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="upper-case($context/child::*[upper-case(local-name()) =upper-case($ColumnName)][1])"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="@caseSensitive=true()">
									<xsl:value-of select="$context/attribute::*[upper-case(local-name())=upper-case($ColumnName)][1]"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="upper-case($context/attribute::*[upper-case(local-name())=upper-case($ColumnName)][1])"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
				<xsl:variable name="meetsCondition">
					<xsl:choose>
						<xsl:when test="ConditionType='contains'">
							<xsl:value-of select="contains($contextColumnValue,$casedColumnValue)"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="ConditionType='equals'">
									<xsl:value-of select="$contextColumnValue=$casedColumnValue"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:choose>
										<xsl:when test="ConditionType='startsWith'">
											<xsl:value-of select="starts-with($contextColumnValue,$casedColumnValue)"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:choose>
												<xsl:when test="ConditionType='endsWith'">
													<xsl:value-of select="ends-with($contextColumnValue,$casedColumnValue)"/>
												</xsl:when>
											</xsl:choose>
										</xsl:otherwise>
									</xsl:choose>
				...
				<!--<xsl:element name="Evaluation">-->
					<xsl:value-of select="$meetsCondition"/>
				<!--</xsl:element>-->
			</xsl:for-each>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="$logicalOperator='AND'">
				<xsl:choose>
					<!--<xsl:when test="not($ConditionsEvaluation[Evaluation=false()])">-->
					<xsl:when test="not(contains(lower-case($ConditionsEvaluation),'false'))">
						<xsl:value-of select="true()"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="false()"/>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="contains(lower-case($ConditionsEvaluation),'true')">
						<xsl:value-of select="true()"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="false()"/>
	...
	<xsl:function name="cfn:componentSpaceForeignKeyMessage" as="xs:string">
		...
		<xsl:value-of select="concat($MsgPrefix,' must exist in ',$FSheet,'.',$FKey,'.  If component is a window or door it may be placed in up to two spaces, but only one otherwise.  Also, you may not use duplicate space names if this component is in more than one space.')"/>
	</xsl:function>
	...
	<xsl:function xmlns="http://purl.oclc.org/dsdl/schematron" name="cfn:delimListInKeys" as="xs:boolean">
		...
		<xsl:choose>
			<xsl:when test="contains($delimString,',')">
				...
				<xsl:choose>
					<xsl:when test="$booleanList[text()='false' or contains(text(),'false')]">
						<xsl:value-of select="false()"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="true()"/>
	...
	<xsl:function xmlns="http://purl.oclc.org/dsdl/schematron" name="cfn:delimPairInKeys" as="xs:boolean">
		...
		<xsl:choose>
			<xsl:when test="contains($delimString,',')">
				<xsl:variable name="tokenizedString" select="tokenize($delimString,',')"/>
				<xsl:variable name="distinctTokens" select="count(distinct-values($tokenizedString))"/>
				<xsl:variable name="tokens" select="count($tokenizedString)"/>
				<xsl:choose>
					<xsl:when test="not($distinctTokens = $tokens)">
						<xsl:value-of select="false()"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:variable name="booleanList">
					...
					<xsl:for-each select="$tokenizedString">
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<xsl:choose>
					<xsl:when test="$booleanList[text()='false' or contains(text(),'false')] or count($booleanList/text())>2">
						<xsl:value-of select="false()"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="true()"/>
	...
	<xsl:function name="cfn:hasAttribute" as="xs:boolean">
		...
		<xsl:variable name="AttributeMatches">
			<xsl:copy-of select="$Context/../../Attributes/Attribute[upper-case(@Name)=upper-case($AttributeName) and upper-case(RowName)=upper-case($RowName) and upper-case(SheetName)=upper-case($SheetName)]"/>
		</xsl:variable>
		<xsl:choose>
	...
	<xsl:function name="cfn:hasAttributeAndValue" as="xs:boolean">
		<!--**This function tests to see if a specific row in a COBie worksheet has a specific attribute in the Attribute worksheet. -->
		<xsl:param name="Context"/>
		<!--**The XPath context of the node to be tested.-->
		<xsl:param name="AttributeName"/>
...
concat(upper-case(substring($x, 1, 1)), lower-case(substring($x, 2))), ' ')"/>
	...
	<xsl:function name="cfn:uniqueNameMessage" as="xs:string">
		...
		<xsl:variable name="RowKey" select="cfn:getKeyValue($Context)"/>
		<xsl:variable name="KeyName" select="concat($SheetName,'Key')"/>
		<xsl:variable name="KeyMatchCount" select="count(key($KeyName,normalize-space(lower-case($RowKey)),$Context/parent::*)) - 1"/>
		
		<xsl:variable name="MsgPrefix" as="xs:string?" select="cfn:assertMsgPrefix($SheetName,$RowKey,$FieldName)"/>
		<xsl:value-of select="concat($MsgPrefix,' must be unique. There are ', $KeyMatchCount, ' other rows with the same unique identifier.  This message will not recur for the other duplicates')"/>
	</xsl:function>
